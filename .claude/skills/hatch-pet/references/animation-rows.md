...
buddy renders from `pets/default/spritesheet.webp` via CSS `background-position` on the
same 8×9 grid. The cell dimensions (192×208) and row/frame counts above are authoritative
for the upstream atlas contract. `package_for_buddy.py` records them in `pet.json` so the
Svelte renderer reads them at runtime without hardcoding.
