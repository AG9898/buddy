...
- **buddy CLI** (`src/cli/`): npm `bin` entry point. Handles `start`, `stop`, `status`, `hooks install`, `state <name>`, `doctor`, `hatch <prompt>`, `pets` (list/current/use, with `show` as an alias of `current`), and `size <scale-or-width>` subcommands, and renders the bare-`buddy` operational overview. The remaining accepted CLI UX refresh adds hook status/uninstall while preserving existing commands. Detects whether it is running in WSL or on Windows and adjusts behavior accordingly — in WSL it launches the Windows Electron app via WSL interop (`buddy.exe`). For asset generation, `buddy hatch` delegates visual generation to a Codex run that can use `$imagegen`, then packages the deterministic hatch-pet outputs for buddy.
