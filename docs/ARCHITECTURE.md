...
- Local HTTP sidecar on `127.0.0.1:7777` (configurable via `BUDDY_PORT`): receives hook events and CLI commands, validates the `X-Petdex-Update-Token` header, and forwards events to main/renderer via Electron IPC. Endpoints: `GET /health` (unauthenticated), `POST /state` (token-authenticated, forwards `CH_STATE_CHANGE` to renderer), `POST /resize` (token-authenticated, forwards `CH_CLI_RESIZE` to main).
