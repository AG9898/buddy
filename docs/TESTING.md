...
| `src/cli/commands/hatch.test.ts` | Hatch command | Numbered hatch stages with elapsed time, TTY-only spinner cleanup, normal/quiet/JSON output routing, normal raw-subprocess suppression, verbose flag/env detail, skill-missing and incomplete-packaging errors, and success summary |
