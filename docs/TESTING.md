...
| `src/cli/output.test.ts` | CLI output layer | status/success/warn/error helpers, NO_COLOR plain mode, check/subCheck/bullet/label, banner text, separator, output modes (normal/quiet/verbose/JSON) with stdout+stderr separation, single-payload JSON success and failure rendering, color resolution for TTY/redirected/FORCE_COLOR/`--no-color` |
