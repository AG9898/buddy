...
| `src/cli/commands/hatch.test.ts` | Hatch command | Numbered hatch stages with elapsed time, TTY-only spinner cleanup, normal/quiet/JSON output routing, normal raw-subprocess suppression, verbose flag/env detail, populated-destination confirmation and non-TTY rejection, Ctrl+C child termination plus allowlisted cleanup, skill-missing and incomplete-packaging errors, and success summary |
...
| `src/cli/commands/pets.test.ts` | Pets commands | Active-pet manifest state resolution with packaged-default fallback; typed `pets.list` / `pets.current` / `pets.use` results: source labels and active-pet marker, invalid-folder counts with reasons kept for verbose/JSON, empty-environment and unresolved-id degradation without failing, selection persistence with preserved state fields, live application against a mocked `POST /pets/use` plus deferred next-start wording when the app is stopped or the token is missing, a `pets.live_rejected` failure that saves nothing when the app refuses the id, typed `pets.id_required` / `pets.not_found` / `pets.invalid` errors with exit code 1, no-writes-to-Codex paths, `BUDDY_DATA_DIR` destination, and quiet/verbose/JSON rendering of the live, deferred, and rejected outcomes |
