...
```bash
SKILL_DIR="${BUDDY_SKILL_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.claude/skills/hatch-pet}"
...
python "$SKILL_DIR/scripts/make_icon.py" \
  --source "$RUN_DIR/references/canonical-base.png" \
  --output "$REPO_ROOT/build/icon.ico"
```
...
For buddy, use `package_for_buddy.py` instead of the shell+jq block below. It writes
buddy's full state-machine `pet.json` (with `frameWidth`, `frameHeight`, and per-frame
`row`/`col`/`ms` entries) rather than the minimal Codex format.

```bash
REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
PET_ID=$(jq -r '.pet_id' "$RUN_DIR/pet_request.json")
python "$SKILL_DIR/scripts/package_for_buddy.py" \
  --run-dir "$RUN_DIR" \
  --output-dir "$REPO_ROOT/pets/default" \
  --pet-id "$PET_ID"
```
...
```bash
PET_DIR="$REPO_ROOT/pets/default"
jq -n \
  --arg run_dir "$RUN_DIR" \
...
## Codex Packaging (upstream reference)

When targeting a standard Codex installation instead of buddy, package with shell and `jq`:
